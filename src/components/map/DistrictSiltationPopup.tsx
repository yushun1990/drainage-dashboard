...
import {
  PipeSiltationHydraulicChart,
  PipeSiltationRiskChart,
  PipeSiltationWaterLevelChart,
} from './charts/PipeSiltationCharts'
...
export function DistrictSiltationPopup({ districtId }: DistrictSiltationPopupProps) {
  ...
  const chartTitles = ['上下游水位差', '流量 / 流速 / 阻力', '淤积风险']
  const buildChartPanelClassName = (index: number) =>
    [
      'drainage-siltation-popup-chart border border-red-400/10 rounded bg-cyan-950/40 overflow-hidden',
  ...
  return (
    <div className="drainage-siltation-popup-panel min-w-[380px] max-w-[440px] text-cyan-50">
      ...
      <div className="shrink-0 border-b border-red-400/10 bg-red-950/20 p-3">
      ...
      <div
        className="drainage-siltation-popup-scroll bg-cyan-950/60 p-3"
        onWheel={(event) => {
      ...
      >
            ...
            {chartTitles.map((title, index) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all ${
                  index === activeChartIndex ? 'w-5 bg-red-300' : 'w-1.5 bg-cyan-500/35'
                }`}
                key={title}
                onClick={() => {
        ...
        <div className="drainage-siltation-popup-chart-stage">
          <div className={buildChartPanelClassName(0)}>
            <PipeSiltationWaterLevelChart data={detail.hydraulicSeries} />
          </div>
          <div className={buildChartPanelClassName(1)}>
            <PipeSiltationHydraulicChart data={detail.hydraulicSeries} />
          </div>
          <div className={buildChartPanelClassName(2)}>
            <PipeSiltationRiskChart data={detail.hydraulicSeries} />
          </div>
        </div>
      </div>
